                                                    ...
                                                    <?php
                                                      ...
                                                      include '../config/koneksi.php';
                                                                 ...
                                                                 if($_SERVER['REQUEST_METHOD'] == "POST") {
                                                                   ...
                                                                   if ($pencarian != '') {
                                                                     $sql = "SELECT id_helpdesk, inbox, outbox FROM tbl_helpdesk WHERE status='0' AND trash='0' AND inbox LIKE '%$pencarian%'";
                                                                     $query = $sql;
                                                                     $queryJml = $sql;
                                                                   } else {
                                                                     $query = "SELECT id_helpdesk, inbox, outbox FROM tbl_helpdesk WHERE status='0' AND trash='0' LIMIT $posisi, $batas ";
                                                                     $queryJml = "SELECT id_helpdesk, inbox, outbox FROM tbl_helpdesk WHERE status='0' AND trash='0'";
                                                                     $no = $posisi + 1;
                                                                   }
                                                                 } else {
                                                                   $query = "SELECT id_helpdesk, inbox, outbox FROM tbl_helpdesk WHERE status='0' AND trash='0' LIMIT $posisi, $batas ";
