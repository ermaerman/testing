                                                    ...
                                                    <?php
                                                      ...
                                                      include '../config/koneksi.php';
                                                                 ...
                                                                 if($_SERVER['REQUEST_METHOD'] == "POST") {
                                                                   ...
                                                                   if ($pencarian != '') {
                                                                     $sql = "SELECT * FROM tbl_helpdesk WHERE status='0' AND trash='0' AND nama LIKE '%$pencarian%' OR email LIKE '%$pencarian%' OR inbox LIKE '%$pencarian%' OR outbox LIKE '%$pencarian%' OR tanggal LIKE '%$pencarian%' ORDER BY id_helpdesk DESC";
                                                                     $query = $sql;
                                                                     $queryJml = $sql;
                                                                   } else {
                                                                     $query = "SELECT * FROM tbl_helpdesk WHERE status='0' AND trash='0' ORDER BY id_helpdesk DESC LIMIT $posisi, $batas ";
                                                                     $queryJml = "SELECT * FROM tbl_helpdesk WHERE status='0' AND trash='0' ORDER BY id_helpdesk DESC";
                                                                     $no = $posisi + 1;
                                                                   }
                                                                 } else {
                                                                   $query = "SELECT * FROM tbl_helpdesk WHERE status='0' AND trash='0' ORDER BY id_helpdesk DESC LIMIT $posisi, $batas ";
                                               ...
                                               if($_SERVER['REQUEST_METHOD'] == "POST") {
                                                  ...
                                                  <div style="float:right;">
                                                    <ul class="pagination pagination-sm" style="margin: 0">
                                                      ...
                                                      for ($i=1; $i <= $jml_hal; $i++) {
                                                        if ($i != $hal) {
                                                          echo "<li><a href=\"index.php?content=belum_dibalas&&hal=$i\">$i</a></li>";
                                                        } else {
