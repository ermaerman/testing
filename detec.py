...
def exec() :
	...
	connection = psycopg2.connect(
	                    database = "mhs",		#ganti dengan koneksi postgre server
	                    user="postgres",
	                    password="12345",
	                    host="localhost",
	                    port="5432")
	...
	PATH_UJI="/home/admins/anaconda3/nanas/wajah/non/"        #'C:/xampp/htdocs/myweb/datalatih/' path folder server
	...
	while (loop<Nloop): 
	    ...
	    print("Tidak ada foto yang belum di proses")
	    time.sleep(1)
...
def cekHitung(face_cascade,eye_cascade,ximage,gray):
    ...
    if n>1 and m>2:
        out=-1
        #print("TIDAK DITEMUKAN WAJAH "+str(n)+"#"+str(m))
    elif n==0 and m==0:
        out=-1
        #print("TIDAK DITEMUKAN WAJAH "+str(n)+"#"+str(m)))
    else:
